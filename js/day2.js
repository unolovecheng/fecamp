...
console.log('문\\자열'); // 문\자열
console.log('문\n자열'); // \n 줄바꿈(new line)
console.log('문\r자열'); // \r 캐리지 리턴(carrigae return)
console.log('문\t자열'); // \t 탭
...
console.log(012) // 8진법 표현, 숫자 제일 앞에 0을 씀
console.log(0x12) // 16진법 표현, 숫자 제일 앞에 0x를 씀

/*
 * 오래된 브라우저에서 문제가 발생하는 것을 막기위해
 * parseInt 두번째 인수에는 사용할 진법을 명시적으로 표시함
 */
console.log(parseInt('012', 10));
// 두 번째 인수를 사용하면 16진수를 10진법으로 변환할 수 있음
console.log(parseInt('0xc5f', 16));

// 숫자를 다른 진법으로 변환할 때는 toString 메서드를 사용함
(35).toString(16); // 10진수 35를 16진수로 변환
35.0.toString(16); 
36..toString(16);
// 모든 같은 결과로 출력됨(숫자 뒤에 .을 찍으면 다른 숫자가 올 것이라 생각함..

//배열
var fruits = ['Apple', 'Banana', 'Cherry']
console.log(fruits[0]); // 배열 이름 뒤에 여는 대괄호, 인덱스 번호, 닫는 대괄호를 입력하면 배열의 원소에 접근할 수 있음

console.log(fruits[3]);
// 없는 인덱스 번호를 사용하면 undefined가 나옴

fruits[3] = 'Mango';
console.log(fruits[3]);

// 중간 인덱스를 건너뛰고 저장하면 중간 인덱스에는 undefined가 저장됨
fruits[5] = 'Melon';
...
// 자신을 기술하는 person 객체를 작성
var person = {
    name : '강창훈',
    age : 30,
    city : '서울',
    cphone : '010-3050-5508',
    email : 'tears_snsanf@naver.com',
    gender : 'male',
    marriage : false,
    '관심사 나열' : ['js', 'css', 'html']
    //변수명 규칙에 반하는 방법의 프로퍼티 명은 ''로 묶어서 사용할 수 있음
    //변수명 규칙을 따른 프로퍼티 명은 ''을 생략할 수 있음
};
...
console.log(person['관심사 나열']);

// 객체 프로퍼티의 접근법2 - 점 문법
console.log(person.name);
//console.log(person.관심사 나열); 점문법에서는 변수명 규칙을 따르지 않은 프로퍼티명은 쓸 수 없음 -> 오류가 남..
...
function 라면요리(라면종류, 물의양, 분){
    console.log(라면종류 + ' 라면을 끓입니다.');
    console.log(물의양 + 'ml 물을 넣고')
    console.log(분+ '분 동안 보글보글');
    console.log('냠냠');
    console.log('끝');
}
라면요리('신라면', 400, 3);
라면요리('너구리', 500, 4);
// 함수를 실행할 때 전달받는 값을 가리켜 인수(argument)라고 함
...
// return 키워드를 사용하면 함수 안의 값을 밖으로 반환(또는 리턴)함
...
// 자바스크립트에서 함수도 하나의 값이기 때문에 다음과 같이 사용할 수 있음
var 제곱1 = 제곱;
console.log(제곱1(5)); //제곱함수를 사용한것과 같은 결과가 나옴
...
// 변수의 스코프(scope) = 유효범위
// 호이스팅(hoisting) - 함수나 변수가 마치 함수를 가장 처음 실행될때 선언된 것처럼 다뤄지는 성질
function 함수() {
...
// 포함하고 있는가
console.log(str.indexOf('마르고')); // 인덱스를 반환
console.log(str.indexOf('d')); // 포함하는 문자열이 없으면 -1를 반환

// 문자열 자르기
var 새문자열 = str.substr(0, 7); // substr(시작위치, 길이);
console.log(새문자열);

var 백두산이 = str.substr(5, 4);
console.log(백두산이);
console.log(str.lastIndexOf('백두산')); // 뒤에서부터 찾음..

// 문자열 자르기 - substring(시작위치, 끝위치); substr과 잘 구분할것.. 

// 숫자의 메서드
var num = 1234.5;

num.toFixed(3); //문자열로 바뀌면서 소수점 3째자리까지 표시됨. 뒤에 자리수가 더 있을경우 반올림함, 없을경우 0으로 채움

// 배열의 메서드
var fruits = ['Apple', 'Banana', 'Cherry'];
console.log(fruits.length);
fruits.push('Mango','Melon','Kiwi');
